...
const LoanApplication = (props) => {
  ...
  const [email, setmail] = useState("mail@som.com");
  ...
  const amtChange = (e) => {
    ...
    var emi =
      (((loanamt * loanrate) / 100) *
        Math.pow(1 + loanrate / 100, loantime)) /
      Math.pow(1 + loanrate / 100, loantime - 1);
    ...
    //  var loanaccid = 1;
      ...
      axios
        ...
        .then((res) => {
          ...
          axios
          .put("http://localhost:8080/addloanschedule", {
            amount:loanamt,
            rate:loanrate,
            month:loantime,
            loanid:loanaccid
